  ...
  try {
    ...
    const enrichedBids = safeBids.map((bid) => {
      ...
      const platformFeePercent = Number(
        bid.platform_fee_percent ?? 10,
      );

      const storedDriverPayout = Number(
        bid.driver_payout ?? 0,
      );
      ...
      const calculatedDriverPayout =
        storedDriverPayout > 0
          ...
          : Number(
              (
                amount *
                (1 - platformFeePercent / 100)
              ).toFixed(2),
            );

      const storedPlatformFee = Number(
        bid.platform_fee ?? 0,
      );
    ...
    const stats = {
      ...
      pending: enrichedBids.filter(
        (bid) =>
          String(bid.status ?? "").toLowerCase() === "pending",
      ).length,

      accepted: enrichedBids.filter(
        (bid) =>
          String(bid.status ?? "").toLowerCase() === "accepted",
      ).length,

      rejected: enrichedBids.filter(
        (bid) =>
          String(bid.status ?? "").toLowerCase() === "rejected",
      ).length,
      ...
      totalPotentialFees: Number(
        enrichedBids
          .reduce(
            (total, bid) =>
              total + bid.financials.platformFee,
            0,
          )
          .toFixed(2),
      ),
    ...
    console.error("Admin bids API error:", error);
